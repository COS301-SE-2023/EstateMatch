...
export class PrivatePropertySaleService {
  ...
  const propertyListings = await Promise.all(firstFivePropertyURLs.map(async (url) => {
    ...
    console.log(secondFivePropertyURLs);
    ...
    // const thirdFivePropertyURLs = propertyURLs.slice(5,10);
    // const thirdPropertyListings = await Promise.all(thirdFivePropertyURLs.map(async (url) => {
    //   // Open a new page for each property
  ...
  // propertyListings.push(...thirdPropertyListings);
